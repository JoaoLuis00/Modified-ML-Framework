...
def pose2numpy(num_frames, poses_list, num_channels=4):
    ...
    for t in range(num_frames):
        data_numpy[0, :, t, :, 0] = np.transpose(poses_list[t].data)
...
def sort_skeleton_data(results):
    ...
    try:
        for i in range(21):
            ...
            hand_keypoints_list.append((c_x,c_y))
    ...
    hand_center_point = (mean_x, mean_y)
...
if __name__ == '__main__':
    ...
    action_classifier = SpatioTemporalGCNLearner(device='cpu', dataset_name='custom', method_name=METHOD,
                                                 in_channels=2,num_point=NUM_KEYPOINTS, graph_type='custom', num_class=9, num_person=1)

    model_saved_path = Path(__file__).parent / 'models' / 'sides_2d' / str(MODEL_TO_TEST) / 'model'
    action_classifier.load(model_saved_path, MODEL_TO_TEST, verbose=True)

    #action_classifier.optimize()

    image_provider = VideoReader(0)  # loading a video or get the camera id 0
    ...
    for img_rgb in image_provider:
        if f_ind % window == 0:
            ...
            pose, hand_center_point, missed_pose, missed_hand = sort_skeleton_data(results)
            counter += 1
            poses_list.append(pose)

            if counter > (TARGET_FRAMES): #if more than 300 frames 
                poses_list.pop(0)
                counter = TARGET_FRAMES
            if counter > 0:
                skeleton_seq = pose2numpy(counter, poses_list,2)
